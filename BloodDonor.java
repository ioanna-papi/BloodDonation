...
public class BloodDonor {
	...
	public static void questionnaire() {
		...
		public static void updateTableAnswers(int qid, String username, String a) {
                	try {
             			int rs = Messages.connect().executeUpdate("UPDATE Answers SET Q_id = '" + qid + "', B_Username = '" + username + "', Answer = '" 
             					+ a +"' WHERE Q_id = '" + qid + "' AND B_Username = '" + username + "'");
                        	Messages.connect().close();
                	} catch (Exception e) {
                        	e.printStackTrace();
                	}
		...
		/**
		 * This method displays messages to donors about the donation days
		 * */
