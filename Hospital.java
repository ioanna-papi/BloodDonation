...
public class Hospital {
	 ...
	 * :param is the kind of update;
	 * limit amount or current blood bank stock*/
	public static double limitLiters(String b, String message) {
         double temp = 0;
         boolean flag = true;
         do{
        	 do {
        		 try {
        			String t = JOptionPane.showInputDialog(null, "Please enter " + message + " of blood for blood type " + b + " :");
	 ...
	 * @param username is the hospital's username*/
		...
		double temp = 0;
         	do {
        		try {
        			String s ="the limit amount";
        			temp = limitLiters(b,s);
                 		if (Answer() == true) {
                			LimitInLiters = temp;
                     			flag = true;
                     			break;
                 		} else {
                			LimitInLiters = limitLiters(b,s);
                			flag = Answer();
                		}
		...
		//Add hospital's blood type limit to the database
                try {
                        int rs = Messages.connect().executeUpdate("INSERT INTO BloodLimits (H_Username, BloodType, BloodLimit)" +
	 ...
	 * This method allows users to sign up to the application*/
        public static void signUp(){
                ...
                do {
                        try {
                                fullname = JOptionPane.showInputDialog(null,"Enter your hospital's name: ", "SIGN UP", JOptionPane.INFORMATION_MESSAGE);
                                flag = true;
		 ...
		 // Hospital's blood limit
                        try {
                                ResultSet rs = Messages.connect().executeQuery("SELECT bloodtype FROM Bloodtypes");
                                while (rs.next()) {
		...
		try {
     		...
     		while (rs.next()) {
            		...
            		double temp = 0;
                     	do {
                    		try {
                    			String s ="your current stock"; 
                    			temp = limitLiters(bloodtype, s);
                             		if (Answer() == true) {
                             				stock = temp;
                                 			flag = true;
                                 			break;
                             		} else {
                            			stock = limitLiters(bloodtype, s);
                            			flag = Answer();
                            		}
                         	} catch(InputMismatchException e2) {
                            	JOptionPane.showMessageDialog(null, "The input has to be a number!", "ALERT MESSAGE", JOptionPane.WARNING_MESSAGE);
            ...
            insertBloodBankStock(username, bloodtype, stock);
     		}
     		rs.close();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static String logIn() {
		boolean flag;
                ...
                JOptionPane.showMessageDialog(null, "Welcome " + hospital_login + "!");
	...
	public static void insertBloodBankStock(String username, String bloodtype, Double blood) {
		try {
			int r = Messages.connect().executeUpdate("INSERT INTO BloodBankStock (H_Username, BloodType, Blood)" +
	...
	public static Double getBloodLimit (String username, String bloodtype) {
		Double bloodlimit = null;
		try {
			ResultSet r = Messages.connect().executeQuery("SELECT * FROM BloodLimits WHERE H_Username ='" + username + "' AND BloodType = '" + bloodtype + "'");
			while (r.next()) {
				bloodlimit = r.getDouble("BloodLimit");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static void updateBloodBankStock(int option, Double amount, String bloodtype, String username) {
		Double blood = null;
		try {
			ResultSet rs = Messages.connect().executeQuery("SELECT * FROM BloodBankStock WHERE H_Username = '" + username +"' AND BloodType = '" + bloodtype + "'");
            while (rs.next()) {
            	...
            	if(blood <= getBloodLimit(username, bloodtype)){
	...
	* @param blood is the current blood bank stock of the given blood type*/
	public static void update (String username, String bloodtype, Double blood) {
		try {
			int rs = Messages.connect().executeUpdate("UPDATE BloodBankStock  SET H_Username = '" + username + "', BloodType ='" 
